...
#
#SBATCH --output=jobs/%a/tout.out
#SBATCH --error=jobs/%a/terror.out
#
...
#
# SBATCH --mail-user='jyeon@udel.edu'
...
#
# [EDIT] Add QuantumEspresso to the environment:
#
vpkg_require qe/6.3

#
# [EDIT] Set QE_APP to the QuantumEspresso program you're using:
#
QE_APP=pw.x

#
# [EDIT] Set QE_INPUT_FILE to your QuantumEspresso input file:
#
QE_INPUT_FILE="PyropeEoS.in"

#
# [EDIT] By default the QuantumEspresso output goes to the Slurm job
#        output file; set QE_OUTPUT_FILE to some filename/path to
#        direct that output elsewhere:
#
QE_OUTPUT_FILE="PyropeEoS.out"
...
#
# Hop into the work directory:
#
if [ -n "$SLURM_ARRAY_TASK_ID" ]; then
    cd jobs/"$SLURM_ARRAY_TASK_ID"
    if [ $? -ne 0 ]; then
        echo "ERROR:  unable to enter job directory for index $SLURM_ARRAY_TASK_ID"
        exit 1
    fi
fi

#
# [EDIT] Execute your QuantumEspresso application:
#
if [ -z "$QE_OUTPUT_FILE" ]; then
    ${UD_MPIRUN} $QE_APP < $QE_INPUT_FILE
    mpi_rc=$?
else
    ${UD_MPIRUN} $QE_APP < $QE_INPUT_FILE > $QE_OUTPUT_FILE
    mpi_rc=$?
fi
